...
var baseTutorialPath = "pwiz_tools/Skyline/Documentation/Tutorials";
var baseOutputPath = "pwiz_tools/Skyline/Documentation/Tutorials/Html";
...
switch (process.argv[2]) {
    case 'convertDocument':
        if (process.argv.length != 5) {
            console.error("Usage: convertDocument <sourceFile> <outputPath>");
            return;
        }
        convertDocument({sourcePath: './', outputPath: process.argv[4], cssPath: "../" + baseCssRelativePath}, process.argv[3]);
        return;
    }
}
console.error("Must specify command: convertDocument");
return;

//convertDocument({sourcePath: './', outputPath: process.argv[3], cssPath: "../../"  + baseCssRelativePath}, process.argv[2]);
...
function convertDirectory(conversion){
    var documentConversionPromises = [];
    fs.mkdirSync(conversion.outputPath, {recursive: true});
