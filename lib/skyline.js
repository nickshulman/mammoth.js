...
function convertDocument(conversion, sourceDocument) {
    ...
    var options = {
        styleMap: [
            ...
            "p.KeepNext => p.keep-next",
            "p.NoSpacing-KeepNext => p.keep-next",
        ...
        transformDocument: transformMammothElement
    };
...
function transformMammothElement(element) {
    if (element.children) {
        var children = _.map(element.children, transformMammothElement);
        element = {...element, children: children};
    }

    if (element.type === "paragraph") {
        element = transformMammothParagraph(element);
    }

    return element;
}

function transformMammothParagraph(element) {
    if (element.keepNext) {
        var styleId = "KeepNext"
        if(element.styleId){
            styleId = element.styleId + "-" + styleId
        }
        return {...element, styleId: styleId};
    } else {
        return element;
    }
}
