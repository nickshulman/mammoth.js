...
function DocumentConversion(options, comments) {
    ...
    function convertParagraph(element, messages, options) {
        var value = htmlPathForParagraph(element, messages).wrap(function() {
    ...
    function htmlPathForParagraph(element, messages) {
        ...
        if (style) {
            ...
            htmlPath = defaultParagraphStyle;
        }
        var clonedHtmlPath = lodash.cloneDeep(htmlPath);
        if(element.keepNext){
            var topLevelElementAttributes = clonedHtmlPath._elements[0].attributes;
            if ("class" in topLevelElementAttributes) {
                topLevelElementAttributes["class"] = topLevelElementAttributes["class"] + " keep-next"
            }else{
                topLevelElementAttributes["class"] = "keep-next"
            }
        }
        return clonedHtmlPath;
    }
    ...
    function defaultTablePath(indent) {
        var path = htmlPaths.elements([
            htmlPaths.element("table", {}, {fresh: true})
        ]);
        if (indent){
            var indentPx = Math.trunc(indent / 17.875);
            path = htmlPaths.elements([
                htmlPaths.element("div", {"style": `margin-left : ${indentPx}px;`}, {fresh: true}),
                htmlPaths.element("table", {}, {fresh: true})
            ]);
        }
        return path;
    ...
    function convertTable(element, messages, options) {
        return findHtmlPath(element, defaultTablePath(element.indent)).wrap(function() {
