...
function BodyReader(options) {
    ...
    function readParagraphProperties(element) {
        return readParagraphStyle(element).map(function(style) {
            var keepNextElement = element.first("w:keepNext");
            var keepNextValue = false;
            if (keepNextElement) {
                if ("w:val" in keepNextElement.attributes){
                    if (keepNextElement.attributes["w:val"] === "1"){
                        keepNextValue = true;
                    }
                } else {
                    keepNextValue = true;
    ...
    function readTableProperties(element) {
        var tableIndentElement = element.first("w:tblInd");
        var tableIndent = 0;
        if (tableIndentElement && "w:w" in tableIndentElement.attributes) {
            tableIndent = tableIndentElement.attributes["w:w"];
        }
